<template>
  <main class="flex-1 relative overflow-y-auto py-6 focus:outline-none" tabindex="0">
    ...
    <div v-show="deleteCompanyModal" class="fixed z-20 inset-0 overflow-y-auto">
      <div
      ...
      >
        ...
        >
          <div v-show="deleteCompanyModal" class="fixed inset-0 transition-opacity">
          ...
          >
            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
              <div class="sm:flex sm:items-start">
                ...
                >
                  <svg-vue
                    class="h-6 w-6 pb-1 text-red-600"
                    icon="font-awesome.exclamation-triangle-light"
                  ></svg-vue>
                </div>
                <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                  ...
                  <div class="mt-2">
                    <p class="text-sm leading-5 text-gray-500">
                      {{ $t("Are you sure you want to delete the company?") }}
                      {{ $t("All data will be permanently removed") }}.
                      {{
                        $t(
                          'Projects with this company will be changed to "Without company"'
                        )
            ...
            <div class="bg-gray-100 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
              ...
              <button
                ...
                type="button"
                @click="deleteCompanyModal = false"
              >
                {{ $t("Cancel") }}
              </button>
            </div>
...
export default {
  ...
  metaInfo() {
    return {
      title: this.$i18n.t("Edit company"),
    };
  },
  data() {
    return {
  ...
  methods: {
    saveCompany() {
      ...
      self.loading = true;
      axios
        .put("api/dashboard/admin/companies/" + self.$route.params.id, self.company)
    ...
    getCompany() {
      ...
      self.loading = true;
      axios
        .get("api/dashboard/admin/companies/" + self.$route.params.id)
        .then(function (response) {
    ...
    deleteCompany() {
      const self = this;
      axios
        .delete("api/dashboard/admin/companies/" + self.$route.params.id)
        .then(function () {
          self.$notify({
            title: self.$i18n.t("Success").toString(),
